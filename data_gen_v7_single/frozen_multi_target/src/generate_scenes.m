...
try
    while accepted<requested && attempted<max_attempts
        ...
        if nt==1
            ...
            indices=draw_clip([cache.recordings(group).source_indices],sampling_uses);
        ...
        for t=1:nt
            ...
            component_paths{slots(t)}=sprintf('components/%s/%s/src_%05d_r%02d_d%02d.wav', ...
                cfg.active_split,cfg.combo_folders{slots(t)+1},indices(t),ranges(t),depths(t));
...
if ~isfile(path)
    audiowrite(path,output,cfg.target_fs,'BitsPerSample',cfg.output_bits);
end
