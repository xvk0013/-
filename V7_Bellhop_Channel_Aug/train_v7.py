"""One original Mamba run with two physical Bellhop views per Train ship."""
import argparse
import copy
import json
import math
...
def main():
    ...
    cfg = Config(**checkpoint["config"]) if checkpoint else Config(**baseline["config"])
    ...
    if (cfg.head_type != "gap" or cfg.band_fusion != "early" or cfg.feature_mixup_prob != 0
            or cfg.spectral_aug_prob != 0 or cfg.local_contrast_bins != 0
            or cfg.ortho_weight != 0 or cfg.cr_weight != 0):
        parser.error("This experiment fixes baseline GAP/early frontend and disables other ablations")
    if cfg.workers < 1:
