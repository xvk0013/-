...
resume_args=()
if [[ -f "runs/single_pool_A_original_ship5_noise1_s42/last.pt" ]]; then
  resume_args=(--resume "runs/single_pool_A_original_ship5_noise1_s42/last.pt")
fi
python train_source_pool.py "${resume_args[@]}" --phase single \
  ...
  --output-dir runs/single_pool_A_original_ship5_noise1_s42 \
