"""Evaluate saved original TF Query EMA on full Train and Val without augmentation or updates."""
import argparse
import csv
import json
...
def main():
    ...
    from eval_helpers import evaluate_variant
    from model import M2Mamba as M2TF
    from train import load_checkpoint, make_loader, seed_all
    saved = load_checkpoint(args.checkpoint)
    from v7_common import AUGMENTATION, TF_PROTOCOL, BN_POLICY
    if saved.get("channel_augmentation") != AUGMENTATION:
        parser.error("Checkpoint is not from this channel augmentation experiment")
    if saved.get("tf_protocol") != TF_PROTOCOL or saved.get("bn_policy") != BN_POLICY:
        parser.error("Use this TF run own best.pt")
    cfg = Config(**saved["config"])
    cfg.workers = args.workers
    cfg.validate()
    if saved["phase"] != "single" or cfg.head_type != "query":
        parser.error("Use the new V11-TF single-target best.pt")
    ...
    model = M2TF(cfg, "single").to(device).eval().requires_grad_(False)
    ...
    result = {"checkpoint": str(args.checkpoint.resolve()), "epoch": saved["epoch"],
              ...
              "precision": "float32", "augmentation": False, "bn_reestimated": True, "bn_reestimated_during_report": False,
              "ema_bn_calibration": saved["ema_bn_calibration"], "bn_policy": BN_POLICY, "tf_protocol": TF_PROTOCOL,
