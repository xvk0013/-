"""One copied V11 TF/Query run on the current V7 Bellhop data."""
import argparse
import copy
import json
import math
...
import random
from v7_common import DATASET, BASELINE_SUMMARY, AUGMENTATION, bootstrap, validate_dataset
import time
bootstrap()
import numpy as np
...
from config import Config, checkpoint_config, from_baseline
from v7_common import TF_PROTOCOL, BN_POLICY
from ema_bn import make_calibration_batches, calibrate_ema
from data import SceneDataset, load_records, check_recording_split, fit_train_stats
from model import M2Mamba as M2TF, parameter_counts
...
def train_epoch(model, loader, optimizer, ema, scaler, device, cfg, global_step):
    ...
    mixup_batches = mixup_ships = alternate_ship_draws = 0
    for step, batch in enumerate(loader, 1):
        ...
        n = len(y)
        alternate_ship_draws += int(batch["alternate_channel"].sum())
    ...
    return {**{k: v / n_samples for k, v in sums.items()},
            "alternate_ship_draws": alternate_ship_draws,
...
from checkpoint_io import save_checkpoint, recover_best
...
def main():
    ...
    if args.phase != "single" or args.data_dir.resolve() != DATASET:
        parser.error("Use this completed V7 dataset in single phase")
    if args.noise_data_dir is None or args.noise_data_dir.resolve() != DATASET:
        parser.error("V7 ship and noise samples must come from the same dataset")
    validate_dataset()
    ...
    cfg = Config(**checkpoint["config"]) if checkpoint else from_baseline(baseline["config"])
    if checkpoint and checkpoint.get("channel_augmentation") != AUGMENTATION:
        parser.error("Resume checkpoint is not from this fixed channel augmentation experiment")
    if checkpoint and (checkpoint.get("tf_protocol") != TF_PROTOCOL or checkpoint.get("bn_policy") != BN_POLICY):
        parser.error("Resume requires this TF experiment own last.pt")
    ...
    if (cfg.head_type != "query" or cfg.band_fusion != "early" or cfg.feature_mixup_prob != 0
            or cfg.spectral_aug_prob != 0 or cfg.local_contrast_bins != 0
            or cfg.ortho_weight != 0 or cfg.cr_weight != 0):
        parser.error("This experiment fixes original TF Query/early frontend and disables other ablations")
    if cfg.workers < 1:
        parser.error("This experiment requires workers >= 1 for epoch-resumable channel RNG")
    cfg.validate()
    print(f"Train physical channel augmentation: {AUGMENTATION}", flush=True)
    ...
    stats = checkpoint["stats"] if checkpoint else baseline["training_record"]["stats"]
    if args.resume:
        recover_best(args.output_dir / "best.pt", checkpoint)
    train_loader = make_loader(ChannelSceneDataset(train_records, cfg, stats, True), cfg, True, device,
    ...
    calibration_batches = make_calibration_batches(train_records, cfg, stats)
    seed_all(cfg.seed)  # Identical initialization regardless of statistics fitting.
    model = M2TF(cfg, args.phase).to(device)
    if checkpoint:
    ...
    if args.resume:
        ...
        committed = [row for row in history_rows if row["epoch"] <= start_epoch]
        if [row["epoch"] for row in committed] != list(range(1, start_epoch+1)):
            raise ValueError("Committed history does not match the resume checkpoint")
        if len(committed) != len(history_rows):
            history_tmp = args.output_dir / "history.recovered.tmp"
            history_tmp.write_text("".join(json.dumps(row, ensure_ascii=False)+"\n" for row in committed),
                                   encoding="utf-8")
            from checkpoint_io import replace_checkpoint
            replace_checkpoint(history_tmp, history_path)
    ...
    config_record = {"config": cfg.to_dict(), "phase": args.phase, "data_dir": str(args.data_dir.resolve()),
                     ...
                     "stats": stats, "parameters": parameter_counts(model), "channel_augmentation": AUGMENTATION,
                     "tf_protocol": TF_PROTOCOL, "bn_policy": BN_POLICY,
    ...
    print(f"M2_TF_V11_CR0_CE copy: {parameter_counts(model)}, phase={args.phase}, scan={cfg.scan_backend}", flush=True)
    ...
    if isinstance(train_loader.sampler, RecordingBalancedSampler):
        print("V7 channel augmentation: each epoch 1800/class + 1080 noise; 5:1; 203 batches; 60 full epochs", flush=True)
    if cfg.scan_backend == "torch":
        print("TF uses CUDA convolutions; EMA BN is calibrated on fixed Train-only data before Val.", flush=True)
    for epoch in range(start_epoch, cfg.epochs):
        ...
        calibration = calibrate_ema(ema.model, calibration_batches, device)
        ...
        row["ema_bn_calibration"] = calibration
        row["source_pool_coverage"] = train_loader.sampler.coverage
        row["epoch_seconds_including_validation"] = time.perf_counter() - epoch_started
        row["peak_cuda_allocated_mib"] = (torch.cuda.max_memory_allocated(device)/2**20
        ...
        payload["channel_augmentation"] = AUGMENTATION
        payload["tf_protocol"] = TF_PROTOCOL
        payload["bn_policy"] = BN_POLICY
        payload["ema_bn_calibration"] = calibration
