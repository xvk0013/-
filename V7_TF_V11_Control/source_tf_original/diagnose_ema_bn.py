...
# Bit order is Tanker, Cargo, Tug; retain all eight possible BCE predictions.
...
@torch.no_grad()
def evaluate_variant(model, loader, device, output_dir, variant):
    model.eval()
        ...
        for step, batch in enumerate(loader, 1):
            ...
            for i in range(size):
                ...
                row = {"path": batch["path"][i], "recording": batch["recording"][i],
                       "truth": set_name(truth), "prediction": set_name(prediction),
            ...
            if step == 1 or step % 25 == 0 or step == len(loader):
                print(f"  {variant} Val: {step}/{len(loader)} batches", flush=True)
    if not n_samples:
    ...
    with (output_dir / f"confusion_{variant}.csv").open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        writer.writerow(["truth / prediction"] + [set_name(code) for code in SET_CODES])
        for code, row in zip(TRUE_CODES, confusion.tolist()):
            writer.writerow([set_name(code)] + row)
    directions = []
    for i, truth in enumerate(TRUE_CODES):
        ...
        for j, prediction in enumerate(SET_CODES):
            ...
            if prediction != truth and count:
                directions.append({"truth": set_name(truth), "prediction": set_name(prediction),
    ...
    print(f"{variant}: EMR={metrics['emr']:.6f}, single EMR={metrics['single']['emr']:.6f}, "
          ...
          f"recall={metrics['single']['recall']}, noise FA={metrics['noise_false_alarm']:.6f}", flush=True)
...
def write_summary(output_dir, report):
    ...
    lines = ["# Online / EMA / Train-only BN reestimation", "",
             ...
             "| Variant | Overall EMR | Ship single EMR | Ship Macro-F1 | Tanker recall | Cargo recall | Tug recall | Noise FA | BCE |",
             "|---|---:|---:|---:|---:|---:|---:|---:|---:|"]
    ...
    lines += ["", "Saved EMA validation metrics are retained in summary.json for reproduction comparison.",
              "A predicted set such as Cargo+Tug is a multiple-positive error on a single ship, "
              "not the same as Cargo -> Tug. Confusion CSV rows are truth and columns are predicted sets."]
    for variant in VARIANTS:
...
def main():
    ...
    train_records = load_records(args.data_dir, "Train", "single")
    val_records = load_records(args.data_dir, "Val", "single")
    ...
    report = {"checkpoint": str(args.checkpoint.resolve()), "epoch": saved["epoch"],
              ...
              "confusion_rows": [set_name(code) for code in TRUE_CODES],
              "confusion_columns": [set_name(code) for code in SET_CODES], "variants": {}}
